...
def test_cache_policy_factory():
    ...
    # 动态 Token 型
    assert isinstance(CachePolicyFactory.get_policy("custom_http_token"), DynamicTokenCachePolicy)
    assert isinstance(CachePolicyFactory.get_policy("client_credentials"), DynamicTokenCachePolicy)
    assert isinstance(CachePolicyFactory.get_policy("authorization_code"), DynamicTokenCachePolicy)
...
def test_dynamic_token_cache_policy():
    """测试 DynamicTokenCachePolicy"""
    policy = DynamicTokenCachePolicy()
    assert policy.should_cache_tool_object() is True
