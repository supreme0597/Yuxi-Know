...
- 新增会话分享功能：对话拥有者可在聊天页生成带有效期的只读分享链接（`/share/{thread_id}?token=...`），匿名访客无需登录即可查看完整消息（含工具调用）、对话状态（agent_state）与交付件/文件，并支持下载交付件；分享可撤销，无效/撤销/过期 token 统一返回 404 且不暴露 owner uid 与反馈。新增 `conversation_shares` 表与 `/api/share/*` 公开只读端点，管理端点挂载于 `/api/chat/thread/{thread_id}/share`。
