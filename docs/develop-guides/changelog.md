...
- 新增内置 `schedules` Skill（参照 `knowledge-base` 范式）：将 7 个 `category="schedules"` 的定时任务工具（`create_schedule` / `update_schedule` / `delete_schedule` / `list_my_schedules` / `get_schedule` / `list_schedule_logs` / `trigger_schedule`）通过 `tool_dependencies` 绑定到该 Skill，由 `SkillsMiddleware` 门控——这些工具不再进通用「工具」选项，而是作为独立 Skill 在智能体配置的「技能」中启用，激活后才对模型可见。启动时 `init_builtin_skills` 自动同步进 `skills` 表。
- 修复定时任务自动触发失效：合并代码时 `run_worker.WorkerSettings` 丢失了 `cron_jobs` 注册（`schedule_poll_job` / `daily_cleanup_schedule_logs_job`），导致 ARQ 永远不会按周期扫描到期任务，仅手动触发（直接入队 `process_agent_run`）可用。恢复 `functions` 与 `cron_jobs` 声明，并移除 `schedule_service` 两处 `create_run(source="schedule")` 的非法关键字参数（来源信息已由 `Message.extra_metadata` 与 `input_payload` 保存）。
- 为定时任务补充引用完整性：`ScheduleDefinition.uid` 增加 `ForeignKey("users.uid")`（与 `UserConfig.uid` 等新约定一致），并在 `ensure_business_schema` 增加幂等迁移块为存量库添加 `fk_schedule_definitions_uid` 约束；同步修正此前遗留的过期索引 `idx_schedule_defs_user` 引用已更名的 `user_id` 列（改为 `uid`），避免全新库初始化时因列不存在而失败。
- 修复定时任务 API 输入校验缺口：① `timezone` 改用 `zoneinfo.ZoneInfo` 校验 IANA 时区有效性，非法值直接 `400`（此前 `compute_next_run` 会静默回退到 `Asia/Shanghai` 而接受脏值）；② 创建/更新时校验 `agent_slug` 必须存在（此前 admin 路径跳过存在性检查，可绑定不存在的 Agent），归属/可见性校验后续统一收敛为 `AgentRepository.get_visible_by_slug`（见下条）；③ 修正调度路由 owner 过滤误用 `current_user.id`（数字）而非存储的 `uid`（登录串），导致非 admin 用户的列表/详情/更新/删除/触发/日志全部匹配不到，统一改为 `str(current_user.uid)`。
- 定时任务权限模型收敛：移除自造的 `verify_agent_access(user_uid, is_admin)` 与 admin/superadmin 角色判定，agent 绑定统一复用 `AgentRepository.get_visible_by_slug`（内部走 `user_can_access_agent`，含共享/全局/部门可见性），router 与 tools 行为一致；`ScheduleRepository` 收敛为纯 CRUD，owner 过滤以可选参数带入 SQL（`get_by_id(id, uid=None)` / `list_schedules(uid=...)`），不再取回后在 Python 比对——定时任务 tools 仅操作本人数据，admin/超管管控范围仅由 HTTP router 负责。新增 `list_agents` 工具（基于 `AgentRepository.list_visible`）供对话中查询当前用户可用的智能体（slug + 名称），解决用户以智能体名称表达、而工具需要 slug 的问题；`list_my_schedules` 更名 `list_schedules`；移除 LLM 无法填写的 `image_content`/`schedule_config` 参数，并为每个工具补充显式 `description`（buildin 的 `*_DESCRIPTION` 范式）；工具 tags 收敛为单个描述性 tag。
- 修复 `list_agents` 工具导致 `GET /api/agent` 500：`list_agents` 未声明显式 `args_schema`，LangChain 按其函数签名自动生成，把 `runtime: ToolRuntime`（langgraph 含 `send: Callable` 字段的 dataclass）纳入入参 schema；Pydantic 无法为其生成 JSON Schema 而抛 `PydanticInvalidForJsonSchema`，使整个工具元数据懒加载失败、级联拖垮智能体详情接口。沿用 `list_kbs` 已验证的范式，为其补充 `ListAgentsInput`（含占位 `dummy` 字段）显式 `args_schema`，将 `runtime` 排除在 schema 之外。
- 清理定时任务表重复索引：`schedule_definitions.uid` / `schedule_logs.schedule_id` / `schedule_logs.created_at` 因模型列 `index=True`（`BusinessBase.metadata.create_all`）与 `ensure_business_schema` 手动 `idx_*` 各建了一份而重复；改为单列索引只由模型 `index=True` 生成（复合轮询索引 `idx_schedule_defs_poll` 保留），迁移中幂等 `DROP INDEX IF EXISTS` 清理存量库的 `idx_schedule_defs_user` / `idx_schedule_logs_schedule` / `idx_schedule_logs_created`。
- 定时任务校验补强：① cron 改为**无条件校验**——create/update（router 与 tools）通过 `croniter.is_valid` 显式校验 `validate_cron`，与启用状态无关（此前仅在校验 `enabled=True` 需重算 `next_run_at` 时才被 `compute_next_run` 顺带校验，禁用任务可保存非法 cron）；② 为 `agent_slug`(64) / `cron_expr`(128) / `timezone`(64) 等 varchar 字段在请求模型与工具输入补充 `max_length`，避免超长值打到数据库 varchar 截断并暴露裸 asyncpg 错误（此前仅 `name` 有长度限制）。
...
- 修复 web Docker 镜像构建失败：`docker/web.Dockerfile` 的 `development` 与 `build-stage` 原使用 `node:24-alpine`（musl libc），而 `npm install -g pnpm@latest` 装上的 pnpm 10 在启动时校验自身 glibc 原生可执行文件 `@pnpm/exe-linux-arm64`，Alpine 上 npm 因 `libc` 不匹配跳过该可选依赖，导致 `pnpm install` 报 "Cannot verify the identity of the @pnpm/exe.linux-arm64 native binary"。将两处基础镜像改为 `node:24-slim`（glibc），与 api/worker 保持一致，`@pnpm/exe` 可正常安装，构建通过。
