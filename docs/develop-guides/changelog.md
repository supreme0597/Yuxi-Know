...
- 修复编辑供应商保存时误清空 API Key 的问题：未输入新 Key 时不提交该字段
- 前端「管理模型」入口按 `can_manage` 显隐，非创建人不再展示可操作的模型管理入口
- 被共享的供应商可只读查看模型列表：响应新增 `can_view` 字段，「管理模型」入口对查看者开放，模型弹窗内增删改 / 拉取远端 / 测试连接仅创建人可见
- 修复消费侧可见性校验失效的问题：`VisibilityCache` 此前从未被填充导致校验形同虚设。现在 run 创建与 `/chat/call` 会按当前用户计算并缓存可见 provider 集合，系统默认模型 / 智能体配置模型 / 对话级覆盖模型均需对用户可见，否则返回 403；resume 继续已授权 run 不重复校验
- 前端请求层不再把所有 403 统一替换为「没有权限执行此操作」：后端有明确 detail（如「无权使用模型: xxx」）时原样透传给用户，无 detail 时才使用通用文案
- 「模型供应商」页签对普通用户开放：可查看共享给自己的供应商及其模型列表（只读），也可创建私有供应商；编辑 / 删除仍仅限创建人
- 供应商相关前端 API 封装移除 admin 前置校验（`getProviders` / 增删改 / 远端模型 / 模型状态改用非 admin 封装），权限改由后端按可见性 / 管理权校验；「刷新缓存」仅 admin 可见
- 模型供应商筛选页签（全部 / 我创建的 / 部门共享 / 全局共享）移入搜索框右侧，与工具页筛选布局保持一致，压缩为工具栏紧凑尺寸
- `ShareConfigForm` 对非管理员不再调用 admin-only 的部门 / 用户列表接口（避免 403）：改为直接列出当前共享范围内的用户（uid），创建人可取消共享；同时编辑供应商时共享范围未变更则不提交，避免后端 `force_private` 静默清空共享
- 模型供应商共享范围新增 `shareAccessLevels` 参数：默认按角色（admin / superadmin 全等级，普通用户仅「指定人」），可传参覆盖为只开放「指定人」
- 模型供应商新增 / 编辑表单改为 `a-form` 内联校验：Provider ID 必填且格式为字母 / 数字 / 下划线 / 中划线（2-100），展示名称与 Base URL 必填，必填字段带红色星号、错误在字段下方即时提示，与后端校验一致
