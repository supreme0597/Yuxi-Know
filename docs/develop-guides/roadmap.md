...
- **MCP 多鉴权编排与动态令牌直连链路**：为 MCP 接入新增 `auth_config_json` 与 `mcp_connections` 绑定模型，支持 `bound_secret`、`custom_http_token`、`client_credentials`、`stdio_env`、`authorization_code` 等鉴权编排基础；后端补齐基于 Redis 的 access token 缓存、预刷新与 401 后自动删缓存重试能力，动态 HTTP MCP 通过 `DynamicMCPTokenAuth` 在请求阶段直接向上游注入令牌；补齐用户/部门绑定连接缺失时的运行时拒绝逻辑，避免个人级 MCP 连接被其他用户串用；同时让管理端 `/api/system/mcp-servers/{name}/tools` 与 `/tools/refresh` 也按当前管理员的 `user_id/department_id` 解析绑定连接，避免跨部门管理员在未授权情况下探测到 MCP 工具列表；新增 Redis 版次 + manifest 分级缓存，让 API/Worker 多进程场景下的 MCP 工具清单按 `server` / `connection` 分区同步失效，并避免旧 graph 中预加载 of managed tool 覆盖本轮实时鉴权加载结果；修复动态 HTTP token 更新后工具对象或长连接未及时使用新令牌、停用 MCP 后运行态缓存未失效、更新 `auth_config` 后 runtime token 未立即清理的问题；统一 Agent 运行态与连接管理页的个人 MCP scope 语义，避免运行态使用数据库主键查找 `mcp_connections.scope_id` 导致个人连接不可用；补齐运行时鉴权 MCP 工具的执行阶段映射，避免模型已绑定 `getTicket` 等动态工具但 ToolNode 静态注册表无法执行的问题；审计并修复该链路隐患：通过 DynamicMCPTokenAuth 引入 15 秒 TTL 在内存缓存（含联动清除机制）解决 httpx 请求对 DB 的高频重复查询问题；修复 `_normalize_token_payload` 处理 naive datetime 的时区偏差问题以消除 token 无限自动刷新的 Bug；改进 `_calculate_config_hash` 哈希计算逻辑，对 json.dumps 增加 default=str 降级保护防止无法序列化而崩溃的问题；优化免密钥连接测试，在 binding_scope 非 inline 且配置未引用 `\${secret.xxx}` 变量时免去 connection 的强检验，允许直接进行测试和工具加载；在 `client_pool` 中实现长连接失效的断线清理机制，防止 anyio.ClosedResourceError 报错固化在缓存中；修复 mock MCP demo server 在 FastAPI 路由下返回值的 ASGI 响应冲突，将其重构为原生 ASGI App 路由，并在 Docker 中容器化部署；前端增加 `${context.work_id}` 快注按键并补齐后端 context.work_id 工号识别支持；修复未配置认证时前端发送空字典 `{}` 导致 Pydantic 400 校验错误的问题。
  - 本次补充：统一 runtime config 的 active connection 强制规则，允许未引用 `${secret.xxx}` 的动态 MCP 无绑定连接运行；连接测试补齐 user scope 的 `work_id`，连接池 hash 忽略逐请求变化的 `Authorization`；补充 MCP 动态鉴权使用说明和开发手册。
  ...
  - 本次补充：修复 MCP 模块迁移后混用新版 `slug`/`uid` 字段与当前分支 `name`/`user_id` 模型导致的 API 启动失败；移除废弃的内部 MCP 代理路由与配置，恢复动态令牌直连及缓存隔离契约，并修复清理 MCP 缓存后连接池被永久关闭的问题。
  - 本次补充：完善 MCP 工具调用断线恢复，长连接由单一 Session 所有者按 generation 主动重建并保留恢复中的连接池代理；通过 Adapter 公共 interceptor 对明确只读或幂等的工具最多安全重试一次，未标注工具不自动重放并返回保守错误；新增真实 Demo Server 重启的集成测试与聊天流 E2E，覆盖断线后当前调用和后续调用的可用性。
  - 本次补充：legacy SSE 后台 reader 发生服务端重启断流时，通过 Adapter 公开 `message_handler` 立即唤醒同一 Session 所有者重连，不再等待最长 30 秒的 ping；已确认的 incomplete chunked read 仅输出简短 WARNING，其他 SDK reader 异常继续保留 ERROR 堆栈。
